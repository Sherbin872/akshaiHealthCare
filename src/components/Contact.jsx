...
const Contact = () => {
  ...
  const mainOffice = {
    icon: MapPin,
    label: "Main Office (Tirunelveli)",
    value:
      "No. 86, Bavani Complex, Iyyappan Nagar,\nNGO 'A' Colony, Tirunelveli - 627 007, Tamil Nadu",
    color: "text-[#DC2626]",
    bgColor: "bg-[#FEF2F2]",
    mapUrl: "https://maps.google.com/?q=8.689302,77.736267",
    embedUrl:
      "https://maps.google.com/maps?q=8.689302,77.736267&z=16&output=embed",
  };

  const branchOffice = {
    icon: Building2,
    label: "Branch Office (Nagercoil)",
    value:
      "No. 50, Vellalar Colony East, Ramavarmapuram,\nNagercoil - 629 001, Kanyakumari District.",
    color: "text-[#9333EA]",
    bgColor: "bg-[#FAF5FF]",
    mapUrl:
      "https://maps.google.com/?q=No.+50,+Vellalar+Colony+East,+Ramavarmapuram,+Nagercoil+-+629001",
    embedUrl:
      "https://maps.google.com/maps?q=No.+50,+Vellalar+Colony+East,+Ramavarmapuram,+Nagercoil+-+629001&z=16&output=embed",
  };

  const dentalHospital = {
    icon: Building2,
    label: "Rathnam Smile Dental Hospital",
    value:
      "577 B, K P Raod, Chettikulam Junction,\nNagercoil - 629 001, Kanyakumari District.",
    color: "text-[#2563EB]",
    bgColor: "bg-[#EFF6FF]",
    mapUrl:
      "https://maps.google.com/?q=577+B,+K+P+Road,+Chettikulam+Junction,+Nagercoil+-+629001",
    embedUrl:
      "https://maps.google.com/maps?q=577+B,+K+P+Road,+Chettikulam+Junction,+Nagercoil+-+629001&z=16&output=embed",
  };
    ...
    try {
      const response = await fetch(FORMSPREE_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          Source: "Akshai Healthcare Services Website - Contact Form",
        }),
      });
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2">
        ...
        <div className="grid lg:grid-cols-5 gap-6 lg:gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-2 space-y-4 animate-slide-in-left">
            <div className="grid grid-cols-3 gap-1 bg-white rounded-xl p-1 shadow-sm border border-gray-100">
              <button
                onClick={() => setActiveTab("main")}
                className={`text-[11px] font-semibold py-2 px-1 rounded-lg transition-all duration-300 ${activeTab === "main" ? "bg-[#1E3A8A] text-white shadow" : "text-gray-500 hover:text-[#1E3A8A]"}`}
              >
                Main Office
              </button>
              <button
                onClick={() => setActiveTab("branch")}
                className={`text-[11px] font-semibold py-2 px-1 rounded-lg transition-all duration-300 ${activeTab === "branch" ? "bg-[#9333EA] text-white shadow" : "text-gray-500 hover:text-[#9333EA]"}`}
              >
                Branch Office
              </button>
              <button
                onClick={() => setActiveTab("dental")}
                className={`text-[11px] font-semibold py-2 px-1 rounded-lg transition-all duration-300 ${activeTab === "dental" ? "bg-[#2563EB] text-white shadow" : "text-gray-500 hover:text-[#2563EB]"}`}
              >
                Dental Hospital
              </button>
            </div>
              ...
              {activeTab === "main" ? (
                  ...
                  {contactInfo.map((item, index) => (
                    <div key={index} className="group flex gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all duration-300">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {item.href ? (
                          <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noopener noreferrer" : undefined} className="text-sm text-gray-700 font-medium hover:text-[#3B82F6] transition-colors break-words">
                            {item.value}
                          </a>
                        ) : (
                        ...
                        {item.action && (
                          <a href={item.href || "#map"} target={item.href?.startsWith("http") ? "_blank" : undefined} rel={item.href?.startsWith("http") ? "noopener noreferrer" : undefined} className="inline-flex items-center gap-1 mt-1 text-[10px] font-semibold text-[#3B82F6] hover:underline">
                ...
                <div className="group flex gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all duration-300">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-700 font-medium whitespace-pre-line mb-1">{branchOffice.value}</p>
                    <a href={branchOffice.mapUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-1 text-[10px] font-semibold text-[#9333EA] hover:underline">
                ...
                <div className="group flex gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all duration-300">
                  <div className={`w-10 h-10 ${dentalHospital.bgColor} rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                    <dentalHospital.icon className={`w-5 h-5 ${dentalHospital.color}`} />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-0.5">{dentalHospital.label}</p>
                    <p className="text-sm text-gray-700 font-medium whitespace-pre-line mb-1">{dentalHospital.value}</p>
                    <a href={dentalHospital.mapUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-1 text-[10px] font-semibold text-[#2563EB] hover:underline">
                      <Navigation className="w-3 h-3" />
                      View on Map
                      <ExternalLink className="w-2.5 h-2.5" />
                    </a>
            ...
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden h-48 sm:h-56">
              <iframe
                title="Akshai Healthcare Services Location"
                src={
                  activeTab === "main"
                    ? mainOffice.embedUrl
                    : activeTab === "branch"
                    ? branchOffice.embedUrl
                    : dentalHospital.embedUrl
                }
                width="100%"
                height="100%"
                style={{ border: 0 }}
